    ...
    for i, claim_config in ipairs(config.claims) do
        ...
        if claim_config.does_not_equal ~= nil then
            if tostring(payload_claim_item) == tostring(claim_config.does_not_equal) then
                return unauthorized_due_to_failed_claim(claim_config.path,
        ...
        if #claim_config.equals_one_of ~= 0 then
            ...
            if not match and check_count > 0 then
                return unauthorized_due_to_failed_claim(claim_config.path, "did not equal one of " ..
                    table.concat(claim_config.equals_one_of, "; "))
            end
        end
        if #claim_config.equals_none_of ~= 0 then
            ...
            if match and check_count > 0 then
                return unauthorized_due_to_failed_claim(claim_config.path, "was equal to one of " ..
                    table.concat(claim_config.equals_none_of, "; "))
            end
        end
        ...
        if #claim_config.contains_one_of ~= 0 then
            if type(payload_claim_item) ~= "table" then
                ...
                if not match and check_count > 0 then
                    return unauthorized_due_to_failed_claim(claim_config.path, "does not contain one of " ..
                        table.concat(claim_config.contains_one_of, "; "))
                end
            end
        end
        if #claim_config.contains_none_of ~= 0 then
            if type(payload_claim_item) ~= "table" then
                ...
                if match and check_count > 0 then
                    return unauthorized_due_to_failed_claim(claim_config.path, "contains one of " ..
                        table.concat(claim_config.contains_none_of, "; "))
                end
            end
        end
